...
class PopupWidget extends StatefulWidget {
  ...
  PopupWidget({Key key,@required this.child,@required this.popupBuilder}):assert(child.key == null ? throw "Popup widget child has no global or local key":true), super(key: key);
...
class _PopupWidgetState extends State<PopupWidget> {
  ...
  _onWidgetTap(BuildContext context) {
    final RenderBox renderBox = (widget.child.key as GlobalKey).currentContext.findRenderObject();
    final size= renderBox.size;
    final position= renderBox.localToGlobal(Offset.zero);
    Widget child = widget.popupBuilder(position,size);
    Navigator.push(context, PopupWidgetModal(child: Container(child: CompositedTransformFollower(link: _layerLink,child: child))));
  }
}
